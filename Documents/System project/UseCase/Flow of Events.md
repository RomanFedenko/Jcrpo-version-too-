...
2. Приложение отображает возможные способы входа в приложение: войти в свою учётную запись;
...
4. Выполняется вариант использования "Отфильтровать новости";
...
**Предусловия.** Был инициирован вариант использования "Загрузить новости".  
**Основной поток.**
1. Приложение получает список новостей из XML-документа;
2. Если список новостей закончился, переход к п.10 основного потока;
3. Если список включений пуст, переход к п.5 основного потока;
4. Приложение проверяет наличие хотя бы одной ключевой фразы из списка включений в описании новости. Если результат отрицательный, переход к п.9 основного потока;
5. Если список исключений пуст, переход к п.7 основного потока;
6. Приложение проверяет наличие хотя бы одной ключевой фразы из списка исключений в описании новости. Если результат положительный, переход к п.9 основного потока;
7. Приложение добавляет новость в стисок отфильтрованных новостей;
8. Приложение переходит к следующей новости в списке новостей;
9.  Возврат к п.2 основного потока;
...
2. Приложение отображает окно настроек профиля, в котором предлагает пользователю выбрать одно из действий: добавить интернет-ресурс, удалить интенет-ресурс, добавить ключевую фразу, удалить ключевую фразу, сохранить настройки;
...
**Постусловия.** Выполняется вариант использования "Загрузить новости".
